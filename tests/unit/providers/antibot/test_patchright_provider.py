...
def test_block_webrtc_reaches_the_solve_function() -> None:
    """docs/PHASE_2_BACKLOG.md item 5: a per-call request reaches the
    real browser-driving call verbatim -- what that call actually does
    with it (log a warning, since Chromium has no equivalent here yet)
    is _default_patchright_solve's own concern, exercised only via
    tests/integration (same "needs a real live browser" reasoning as
    every other real behavior in that function)."""
    seen: dict[str, bool] = {}
