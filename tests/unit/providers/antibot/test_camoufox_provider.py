...
def test_exhausted_browser_crash_records_a_network_infra_transient_failure(
    ...
    monkeypatch.setattr(
        "src.providers.antibot.camoufox_provider.record_failure",
    ...
    provider = CamoufoxProvider(solve_fn=always_crashes, max_browser_crash_attempts=2)
...
def test_a_retried_then_recovered_crash_records_nothing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Failure-adjacent case: a crash that recovers on retry must never
    reach the failure registry -- only an attempt that actually
    exhausted every retry is a real, final failure."""
    recorded: list[FailureRecord] = []
    monkeypatch.setattr(
        "src.providers.antibot.camoufox_provider.record_failure",
        lambda record, path=None: recorded.append(record),
