...
| DOM Virtualization (قوائم افتراضية) | مش كل العناصر موجودة في الـ DOM في نفس اللحظة، حتى لو ظاهرة على الشاشة | ✅ مُختبر - **Known Limitation حقيقية مؤكدة بدليل CI لـ الاتنين معًا** (docs/REQUIREMENTS.md قسم 9 بند 13، CI run 32725098955: `parsed_html` و`live_dom` الاتنين بيرجّعوا 5 items بالظبط بس) - أول جولة يبان فيها إن `live_dom` (حل بند 12) مش حل عام: بيحل مشاكل *encapsulation* بس، مش مشاكل *وجود/توقيت* |
