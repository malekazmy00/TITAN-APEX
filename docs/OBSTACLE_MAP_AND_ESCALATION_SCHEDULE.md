...
| DOM Virtualization (قوائم افتراضية) | مش كل العناصر موجودة في الـ DOM في نفس اللحظة، حتى لو ظاهرة على الشاشة | 🔶 مُبني، قيد التأكيد بـ CI حقيقي (docs/REQUIREMENTS.md قسم 9 بند 13) - Known Limitation متوقعة لـ **الاتنين** (`parsed_html` و`live_dom`) - مشكلة توقيت + عدم وجود فعلي في الـ DOM، مش نوع الـ selector |
